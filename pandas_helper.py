...
class Concepts:
    ...
    LATTICE_TOWER = "Q1440476"
    WONDERS_OF_THE_ANCIENT_WORLD = "Q66108498"
    DOMESTICATED_ANIMAL = "Q57814795"
...
def load_data(datapath, usecols=None, limit=None):
    ...
    df["publication_date"] = pd.to_datetime(df["publication_date"], unit="s", errors="coerce")
...
class WikilanguageAccessor:
    ...
    def best_concepts(self, sample=0.1, n=200, direct=False):
        ...
        prob_mass = (
            pd.concat((instances_of, self._df.loc[instances_of.index]["enwiki_pagerank"]), axis=1, copy=False,)
            .groupby([key])
            .sum()
            .nlargest(n, "enwiki_pagerank")
        )
        ret = pd.concat((self._df.loc[prob_mass.index]["sample_label"], prob_mass), axis=1, copy=False).dropna()
        return ret

    def top_ranked(self, wiki, n=200, desc=True):
        ret = self._df[self._df[f"{wiki}_title"].notna()][["sample_label", f"{wiki}_pagerank"]]
    ...
    def kl_divergence(self, base_wiki, target_wiki, n=200, marginals=True, importance_weight=1):
        ...
        matching_rows["kl_divergence"] = (matching_rows[f"{target_wiki}_pagerank"] ** importance_weight) * (
        ...
        matching_rows["kl_relative_to_max"] = matching_rows["kl_divergence"] / matching_rows["kl_divergence"].max()
