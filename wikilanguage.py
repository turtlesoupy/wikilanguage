...
def main():
    ...
    try:
        for path in wiki_paths:
            ...
            shelf_working_path = os.path.join(working_dir, f"{wikiname}") if working_dir else None
            ...
            if shelf_working_path and glob.glob(f"{shelf_working_path}*"):
                ...
                pipelines.write_articles_to_shelf(shelf, path, rank_in_memory=in_memory, limit=limit)
                if shelf_working_path:
                    for desc in glob.glob(f"{temp.name}*"):
                        _, ext = os.path.splitext(desc)
                        print(f"Main: copying shelf {desc} to {shelf_working_path}{ext}")
        ...
        inheritance_working_path = os.path.join(working_dir, f"inheritance.pickle") if working_dir else None
        if inheritance_working_path and os.path.exists(inheritance_working_path):
            ...
            inheritance_graph = pipelines.wikidata_inheritance_graph(wikidata_path, limit=limit)
        ...
        pipelines.write_csv(
            wikidata_path, output_path, wiki_shelves, parent_finder, limit=limit, whitelisted_wikis=whitelisted_wikis,
        )
